! ======================================================================
! User Subroutine UEL for Abaqus: two and three elements for the split scheme
!  operator phase and displacement problem:
...
!
! ---- Used variables ---------------------
! N_ELEM - number of elements used in the model divided
!            by 3 - (N_phase+N_stress+N_UMAT)/3 (to be changed for each model)
!
! NSTVTT - solution dependent variables for the displacement element
!            (displacements, strains, stresses,energies, phase, etc.)
! NSTVTO - solution dependent variables for the phase field element
!            (phase, energy history)
! NSTV - overall solution dependent variables (NSTVTO+NSTVTT+4), where
!           the additional 4 variables are the: time and iteration number
!
!     ==================================================================
...
!     ==================================================================
     ...
     2 SIX=6.D0,SEVEN=7.D0,EIGHT=8.D0,PONE=1.5D0,DTMIN=1.0D-6,
     3 DPHMIN=1.0D-2,NSTVTO=2,NSTVTT=3,NSTV=9,N_ELEM=15581)
!     ==================================================================
...
!     ==================================================================
     ...
     3 VNI(MCRD,NDOFEL),ULOC(MCRD),PHASENOD(NNODE),PNEWDTIP(8),EIGV(3),
     4 ALPHAI(3),VECTI(3),EPSC(6),PHMAX(8),PHMAX0(8),DPHMAX(8)
     ...
     3 ENGP,ENGN,ATPAR,ENGKG,ENGEG,PHELEMAX,DPHELEMAX
...
!		
        ...
        DPHELEMAX=ZERO		
        DPHMAX(INPT)=ZERO
        DPHMAX(INPT)=PHMAX(INPT)-PHMAX0(INPT)
        DPHELEMAX=MAXVAL(DPHMAX)
        IF (DPHELEMAX.LT.ZERO) THEN
         DPHELEMAX=ZERO
        ENDIF
!
        DPHASE0=ZERO
...
!        SDV(23)=ENG
        ...
        ENGKG=ENGKG+(ENG*OMEGA)*DTM*THCK	
...
!     ==================================================================
!
!---------  Phase field increment and time step control -------------
!		
! --------- If automatic time integration is used
        IF ((DPHASE0.GT.DPHMIN).AND.(REITER.LT.4)) THEN  
! --------- If phase field criterion is satisfied		
           IF ((PHASE.GE.0.3D0).AND.(PHASE.LT.0.6D0)) THEN !PHELEMAX
! --------- If time field criterion is satisfied 
             IF (DTIME.GT.DTMIN*10.D0*(ONE+TOLER)) THEN
! --------- If time-step still can be reduced
               PNEWDTIP(INPT)=0.1D0
               DTIMENEXT=PNEWDTIP(INPT)*DTIME
               IF (DTIMENEXT.LE.DTMIN*10.D0*(ONE+TOLER)) THEN
                 PNEWDTIP(INPT)=TWO
               ENDIF
             ELSE
! --------- If time-step is already too small
               PNEWDTIP(INPT)=TWO
             ENDIF		   
           ELSEIF ((PHASE.GE.0.6D0).AND.(PHASE.LT.0.9D0)) THEN
! --------- If time field criterion is satisfied 
             IF (DTIME.GT.DTMIN*10.D0*(ONE+TOLER)) THEN
! --------- If time-step still can be reduced
               PNEWDTIP(INPT)=0.1D0
...
! --------- If time-step is already too small
               PNEWDTIP(INPT)=TWO
             ENDIF			 			 
           ELSEIF ((PHASE.GE.0.9D0).AND.(PHASE.LE.1.0D0)) THEN
! --------- If time field criterion is satisfied 
             IF (DTIME.GT.DTMIN*(ONE+TOLER)) THEN
! --------- If time-step still can be reduced
               PNEWDTIP(INPT)=0.1D0
               DTIMENEXT=PNEWDTIP(INPT)*DTIME
               IF (DTIMENEXT.LE.DTMIN*(ONE+TOLER)) THEN
                 PNEWDTIP(INPT)=TWO
               ENDIF
             ELSE
! --------- If time-step is already too small
...
! 
     ...
     1 HALF=0.5D0,N_ELEM=15581,NSTV=9) 
!	 
